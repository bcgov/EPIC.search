...
def process_files(project_id, file_keys, metadata_list, api_docs_list, batch_size=4, temp_dir=None):
    ...
    with ProcessPoolExecutor(max_workers=batch_size) as executor:
        ...
        for i, (file_key, base_meta, api_doc) in enumerate(tasks):
            ...
            future_to_task[future] = (file_key, base_meta, api_doc, worker_id, doc_name)
            
            # Only register the first batch_size documents as active (they start immediately)
            if i < batch_size:
                progress_tracker.start_document_processing(worker_id, doc_name)
        
        completed_count = 0
        
        # Process completed tasks
        for future in as_completed(future_to_task):
            file_key, base_meta, api_doc, worker_id, doc_name = future_to_task[future]
            ...
            try:
                ...
                if result is None:
                    ...
                    progress_tracker.finish_document_processing(worker_id, success=True)
            ...
            if next_index < len(tasks):
                ...
                if next_index < len(futures_list):
                    ...
                    if not next_future.done():  # Make sure it hasn't completed already
                        _, _, _, next_worker_id, next_doc_name = future_to_task[next_future]
                        progress_tracker.start_document_processing(next_worker_id, next_doc_name)
